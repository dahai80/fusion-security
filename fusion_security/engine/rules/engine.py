...
class ScanRule:
    ...
    def to_rule(self):
        ...
        return Rule(
            ...
            severity=self.severity, cwe_id=self.cwe_id, pattern=self.pattern,
            language=self.language, fix_template=self.fix_template,
            category=self.category, prdid=self.prdid,
        )
...
class RuleEngine:
    ...
    def _init_rules(self) -> None:
        rules = [
            ...
            ScanRule("EVAL001", "表达式注入", "检测到eval/exec执行用户可控输入", "critical", "CWE-94",
                     r"(?i)(eval\s*\(|exec\s*\(|new\s+Function\s*\(|SpEL|OGNL|ExpressionParser)",
                     fix_template="禁止使用eval/exec执行用户输入，使用安全的解析方式",
                     category="injection", prdid="FUS-INJ-005"),
            ScanRule("LDAP001", "LDAP注入", "检测到LDAP查询拼接用户输入", "medium", "CWE-90",
                     r"(?i)(ldap.*search|ldap.*query|ldap.*filter\s*.*\+|search_s\s*\(|search_ext\s*\()",
                     fix_template="使用参数化LDAP查询，对用户输入进行LDAP特殊字符转义",
                     category="injection", prdid="FUS-INJ-006"),
            ScanRule("SSTI001", "模板注入", "检测到服务端模板引擎接收用户可控内容", "critical", "CWE-94",
                     r"(?i)(render_template_string\s*\(|Jinja2.*render|Template\s*\(.*\+|\.render\s*\(.*request|Mako.*template)",
                     fix_template="使用固定模板字符串，不将用户输入作为模板内容",
                     category="injection", prdid="FUS-INJ-007"),
            # ===== 新增规则: 认证类 =====
            ScanRule("WEAKPWD001", "弱密码策略", "检测到密码复杂度校验缺失", "medium", "CWE-521",
                     ...
                     category="auth", prdid="FUS-AUTH-002"),
            ScanRule("JWT001", "JWT配置缺陷", "检测到JWT签名校验禁用或弱密钥", "high", "CWE-327",
                     ...
                     category="auth", prdid="FUS-AUTH-003"),
            ScanRule("SESSFIX001", "会话固定攻击", "检测到登录后未重新生成会话ID", "medium", "CWE-384",
                     ...
                     category="auth", prdid="FUS-AUTH-004"),
            ScanRule("CAPTCHA001", "验证码缺失", "检测到登录/注册接口无验证码保护", "medium", "CWE-307",
                     ...
                     category="auth", prdid="FUS-AUTH-005"),
            # ===== 新增规则: 访问控制类 =====
            ScanRule("UPLOAD001", "不安全文件上传", "检测到文件上传功能未校验文件类型", "high", "CWE-434",
                     ...
                     category="acl", prdid="FUS-ACL-004"),
            ScanRule("BYPASS001", "接口权限绕过", "检测到鉴权中间件存在绕过路径", "high", "CWE-862",
                     ...
                     category="acl", prdid="FUS-ACL-005"),
            # ===== 新增规则: 敏感数据类 =====
            ScanRule("PLAINTEXT001", "敏感数据明文存储", "检测到密码等敏感信息明文存储", "high", "CWE-312",
                     ...
                     category="data", prdid="FUS-DATA-001"),
            ScanRule("LOGLEAK001", "日志泄露敏感信息", "检测到日志输出包含密码/Token等敏感数据", "medium", "CWE-532",
                     ...
                     category="data", prdid="FUS-DATA-002"),
            ScanRule("RESLEAK001", "接口返回敏感信息", "检测到接口返回多余敏感字段", "medium", "CWE-200",
                     ...
                     category="data", prdid="FUS-DATA-003"),
            # ===== 新增规则: 密码学类 =====
            ScanRule("INSECRAND001", "不安全随机数", "检测到使用伪随机数生成安全敏感数据", "medium", "CWE-338",
                     r"(?i)(random\.(random|randint|choice|shuffle)|Math\.random\(\))(?!.*#.*not.security)",
                     fix_template="使用secrets模块或os.urandom()生成Token/密钥等安全敏感随机数",
                     category="crypto", prdid="FUS-CRYPTO-003"),
            ScanRule("SSLVERIFY001", "SSL证书校验禁用", "检测到HTTPS请求禁用服务端证书校验", "high", "CWE-295",
                     ...
                     category="crypto", prdid="FUS-CRYPTO-004"),
            # ===== 新增规则: 安全配置类 =====
            ScanRule("DEFPASS001", "默认账号密码", "检测到使用默认账号密码配置", "high", "CWE-798",
                     ...
                     category="config", prdid="FUS-CONF-001"),
            ScanRule("CORS001", "CORS配置过宽", "检测到跨域配置允许任意来源", "medium", "CWE-942",
                     r"(?i)(Access-Control-Allow-Origin\s*:\s*\*|CORS.*origin.*\*|allow_origins\s*=\s*\[.*\*|cors.*\*.\*origin)",
                     fix_template="限制CORS允许的来源为已知域名白名单",
                     category="config", prdid="FUS-CONF-003"),
            ScanRule("HEADER001", "安全响应头缺失", "检测到缺少安全HTTP响应头配置", "low", "CWE-693",
                     ...
                     category="config", prdid="FUS-CONF-004"),
            # ===== 新增规则: 反序列化/SSRF/CSRF =====
            ScanRule("DESER001", "不安全反序列化", "检测到反序列化用户可控数据", "critical", "CWE-502",
                     ...
                     category="injection", prdid="FUS-DESER-001"),
            ScanRule("SSRF001", "SSRF服务端请求伪造", "检测到服务端请求用户可控URL", "high", "CWE-918",
                     ...
                     category="injection", prdid="FUS-SSRF-001"),
            ScanRule("CSRF001", "CSRF跨站请求伪造", "检测到状态变更接口无CSRF Token校验", "medium", "CWE-352",
                     r"(?i)@router\.(post|put|delete|patch)\s*\(['\"]/(user|account|password|transfer|delete|admin)",
                     fix_template="为状态变更接口添加CSRF Token校验",
                     category="acl", prdid="FUS-CSRF-001"),
            # ===== 新增规则: CRYPTO-002 / CONF-005 / DATA-004 =====
            ScanRule("CRYPTO002", "硬编码加密密钥", "检测到代码中硬编码的加密密钥或AES密钥", "high", "CWE-321",
                     r"(?i)(encryption_key\s*=\s*['\"][^'\"]{8,}|aes_key\s*=\s*['\"][^'\"]{8,}|secret_key\s*=\s*['\"][^'\"]{10,}|private_key_pem\s*=\s*['\"][^'\"]{20,})",
                     fix_template="使用密钥管理服务(KMS)或环境变量存储加密密钥",
                     category="crypto", prdid="FUS-CRYPTO-002"),
            ScanRule("DIRTRAVERS001", "目录遍历开放", "检测到静态文件或目录列表功能未限制路径", "medium", "CWE-548",
                     ...
                     category="config", prdid="FUS-CONF-005"),
            ScanRule("INSECURETRANS001", "不安全数据传输", "检测到使用HTTP传输敏感数据或禁用TLS校验", "high", "CWE-319",
                     ...
                     category="data", prdid="FUS-DATA-004"),
        ]
