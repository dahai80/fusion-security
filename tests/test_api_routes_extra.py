...
def _make_project_orm(**overrides):
    defaults = dict(
        id="proj001", name="test-project", repo_url="",
...
def _make_scan_orm(**overrides):
    defaults = dict(
        id="scan001", project_id="proj1", scan_type="full",
        status="pending", severity_threshold="low", use_ai=True,
        model="", trigger="manual", branch="",
        base_commit="", head_commit="",
        files_scanned=0, files_skipped=0, duration_ms=0.0,
        total_vulnerabilities=0, critical=0, high=0,
        medium=0, low=0, summary="",
    )
    defaults.update(overrides)
    orm = MagicMock()
    for k, v in defaults.items():
...
def _make_vuln_orm(**overrides):
    defaults = dict(
        id="v001", title="SQL Injection", description="test",
        severity="high", confidence=0.9, file_path="test.py",
        line_number=10, code_snippet="code", rule_id="SQL001",
        cwe_id="CWE-89", fix_suggestion="fix it", verified=False,
        status="open", data_flow_path="",
    )
    defaults.update(overrides)
    orm = MagicMock()
    for k, v in defaults.items():
...
def _make_patch_orm(**overrides):
    defaults = dict(
        id="patch001", vuln_id="v001", scan_id="scan001",
        diff_content="diff", original_code="orig",
        patched_code="patched", description="desc",
        status="pending", strategy="template", verified=False,
    )
    defaults.update(overrides)
    orm = MagicMock()
    for k, v in defaults.items():
...
class TestScansCreate:
    ...
    def test_create_scan_no_path(self, mock_scan_to_orm, mock_run, override_client, mock_db):
        ...
        resp = override_client.post("/api/v1/scans", json={
            "project_id": "proj1",
            "path": "",
            "scan_type": "full",
        })
...
class TestScansIncremental:
    ...
    def test_incremental_scan_not_git_repo(self, override_client, mock_db):
        with patch("fusion_security.engine.vcs.git.GitHelper", side_effect=ValueError("not a git repo")):
            resp = override_client.post("/api/v1/scans/incremental", json={
                "path": "/tmp/not_a_repo",
                "base": "HEAD~1",
                "head": "HEAD",
            })
...
class TestScansQueue:
    ...
    def test_enqueue_scan(self, mock_scan_to_orm, override_client, mock_db):
        ...
        mock_scan_to_orm.return_value = mock_orm
        resp = override_client.post("/api/v1/scans/queue", json={
            "project_id": "proj1",
            "path": "/tmp/test",
            "scan_type": "full",
            "priority": 2,
    ...
    def test_cancel_queue_task_found(self, mock_scan_to_orm, override_client, mock_db):
        mock_orm = _make_scan_orm(status="queued")
        mock_scan_to_orm.return_value = mock_orm
        enq = override_client.post("/api/v1/scans/queue", json={
            "project_id": "proj1",
            "path": "/tmp/test",
        })
...
class TestScansCheckpointsAndResume:
    ...
    def test_resume_scan_checkpoint_not_found(self, mock_run, override_client, mock_db):
        with patch("fusion_security.engine.resume.CheckpointManager") as MockCP:
            ...
            resp = override_client.post("/api/v1/scans/resume", json={
                "scan_id": "nonexistent",
                "path": "/tmp/test",
            })
            assert resp.status_code == 404
...
class TestReportsGenerate:
    ...
    def test_generate_report_md_format(self, override_client, mock_db):
        ...
        resp = override_client.post("/api/v1/reports/generate", json={
            "scan_id": "scan001",
            "format": "md",
        })
        assert resp.status_code == 200
...
class TestProjectsCRUD:
    ...
    def test_create_project(self, mock_project_to_orm, override_client, mock_db):
        ...
        resp = override_client.post("/api/v1/projects", json={
            "name": "test-project",
    ...
    def test_list_projects_with_status_filter(self, override_client, mock_db):
        mock_orm = _make_project_orm()
        mock_db.query.return_value.filter.return_value.offset.return_value.limit.return_value.all.return_value = [mock_orm]
...
class TestPatchesCRUD:
    ...
    def test_list_patches_with_filters(self, override_client, mock_db):
        ...
        resp = override_client.get("/api/v1/patches", params={
            "vuln_id": "v001",
            "scan_id": "scan001",
            "status": "pending",
        })
    ...
    def test_update_patch_not_found(self, override_client, mock_db):
        mock_db.query.return_value.filter.return_value.first.return_value = None
        resp = override_client.patch("/api/v1/patches/nonexistent", json={
            "status": "reviewed",
        })
        assert resp.status_code == 404
    ...
    def test_update_patch_found(self, override_client, mock_db):
        mock_orm = _make_patch_orm()
        mock_db.query.return_value.filter.return_value.first.return_value = mock_orm
        resp = override_client.patch("/api/v1/patches/patch001", json={
            "status": "reviewed",
            "verified": True,
        })
        assert resp.status_code == 200
...
class TestVulnerabilitiesCRUD:
    ...
    def test_list_vulnerabilities_with_filters(self, override_client, mock_db):
        ...
        mock_db.query.return_value.filter.return_value.order_by.return_value.offset.return_value.limit.return_value.all.return_value = [mock_orm]
        resp = override_client.get("/api/v1/vulnerabilities", params={
            "severity": "high",
            "status": "open",
            "rule_id": "SQL001",
            "file_path": "test.py",
    ...
    def test_recent_findings_with_hours(self, override_client, mock_db):
        mock_orm = _make_vuln_orm()
        mock_db.query.return_value.filter.return_value.order_by.return_value.limit.return_value.all.return_value = [mock_orm]
...
class TestIntegrationsGate:
    ...
    def test_evaluate_gate_with_all_fields(self, client):
        resp = client.post("/api/v1/integrations/gate", json=[
            {
                "id": "v1", "title": "SQL Injection", "description": "sql inject",
...
class TestIntegrationsCVSS:
    ...
    def test_calculate_cvss_custom_params(self, client):
        resp = client.post("/api/v1/integrations/cvss", params={
            "av": "A", "ac": "H", "pr": "L", "ui": "R",
            "s": "C", "c": "H", "i": "H", "a": "H",
        })
        assert resp.status_code == 200
...
class TestIntegrationsFeedback:
    ...
    def test_add_feedback(self, client):
        resp = client.post("/api/v1/integrations/feedback", params={
            "vuln_id": "v1",
            "rule_id": "SQL001",
            "file_path": "test.py",
            "line_number": 10,
    ...
    def test_feedback_stats(self, client):
        client.post("/api/v1/integrations/feedback", params={
            "vuln_id": "v2",
            "rule_id": "XSS001",
            "file_path": "test.py",
            "line_number": 5,
...
class TestIntegrationsCustomRules:
    ...
    def test_create_custom_rule(self, client):
        resp = client.post("/api/v1/integrations/rules", params={
            "id": "custom100",
            "name": "My Rule",
            "pattern": "eval\\(",
            "severity": "high",
    ...
    def test_list_custom_rules(self, client):
        client.post("/api/v1/integrations/rules", params={
            "id": "custom200",
            "name": "Rule 2",
            "pattern": "exec\\(",
        })
...
class TestIntegrationsNotifications:
    ...
    def test_add_feishu_notifier(self, client):
        resp = client.post("/api/v1/integrations/notify/feishu", json={
    ...
    def test_add_dingtalk_notifier(self, client):
        resp = client.post("/api/v1/integrations/notify/dingtalk", json={
    ...
    def test_send_notification_no_channels(self, client):
        with patch("fusion_security.api.routes.integrations._notification_dispatcher", None):
            resp = client.post("/api/v1/integrations/notify/send", json={
    ...
    def test_send_notification_with_feishu(self, client):
        client.post("/api/v1/integrations/notify/feishu", json={
            "webhook_url": "https://open.feishu.cn/open-apis/bot/v2/hook/test",
        })
    ...
    def test_send_notification_with_dingtalk(self, client):
        client.post("/api/v1/integrations/notify/dingtalk", json={
            "webhook_url": "https://oapi.dingtalk.com/robot/send?access_token=test",
        })
    ...
    def test_get_current_key_invalid_returns_401(self):
        ...
        with pytest.raises(Exception) as exc_info:
            asyncio.get_event_loop().run_until_complete(
                get_current_key(None, api_key="fs_bad_key")
            )
        assert exc_info.value.status_code == 401
